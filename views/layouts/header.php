...
<?php if ($currentUser): ?>
<!-- Global Desktop Navigation (>= 992px only) -->
<nav class="app-navbar d-none d-lg-block">
    <div class="container-fluid d-flex justify-content-between align-items-center" style="max-width: 1200px;">
        <a href="<?= url('/') ?>" class="brand text-decoration-none">
            ...
            <span class="text-white fw-bold">GI ORDER POS</span>
            <?php if (!empty($currentUser['restaurant_name'])): ?>
                <span class="text-white-50 fw-normal" style="font-size: 0.85rem;">| <?= e($currentUser['restaurant_name']) ?></span>
            <?php endif; ?>
            <span class="badge-role"><?= strtoupper(e($currentUser['role'])) ?></span>
        </a>
        ...
        <div class="app-nav-links">
            ...
            <?php elseif ($currentUser['role'] === User::ROLE_MANAGER): ?>
                ...
                <a href="<?= url('manager/settings') ?>" class="<?= str_contains($currentUri, 'manager/settings') ? 'active' : '' ?>">Profile</a>
            ...
            <a href="<?= url('logout') ?>" class="text-danger fw-bold ms-2">Logout</a>
        </div>
    </div>
</nav>
<?php endif; ?>
